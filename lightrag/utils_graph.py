...
def _stable_test_id(name: str) -> str:
    return f"test:{name}"


async def get_test_items_by_equipment_report(
    knowledge_graph_inst,
    equipment_name: str,
    report_type: str,
) -> list[dict[str, Any]]:
    equip_id = _stable_equipment_id(equipment_name)
    report_id = _stable_report_id(report_type)

    equip_edges = await knowledge_graph_inst.get_node_edges(equip_id)
    report_edges = await knowledge_graph_inst.get_node_edges(report_id)
    if not equip_edges or not report_edges:
        return []

    equip_reports = set()
    for src, tgt in equip_edges:
        edge = await knowledge_graph_inst.get_edge(src, tgt)
        if edge and edge.get("rel_type") == "REQUIRES_REPORT" and edge.get("src_id") == equip_id:
            equip_reports.add(edge.get("tgt_id"))

    if report_id not in equip_reports:
        return []

    test_ids: list[str] = []
    for src, tgt in report_edges:
        edge = await knowledge_graph_inst.get_edge(src, tgt)
        if edge and edge.get("rel_type") == "INCLUDES_TEST" and edge.get("src_id") == report_id:
            test_ids.append(edge.get("tgt_id"))

    nodes = await knowledge_graph_inst.get_nodes_batch(test_ids)
    return [node for node in nodes.values() if node is not None]


async def get_params_by_test_item(
    knowledge_graph_inst,
    test_item: str,
) -> list[dict[str, Any]]:
    test_id = _stable_test_id(test_item)
    ...
    for src, tgt in edges:
        edge = await knowledge_graph_inst.get_edge(src, tgt)
        if edge and edge.get("rel_type") == "HAS_PARAMETER" and edge.get("src_id") == test_id:
            param_ids.append(edge.get("tgt_id"))

    nodes = await knowledge_graph_inst.get_nodes_batch(param_ids)
    return [node for node in nodes.values() if node is not None]


async def get_clauses_by_test_item(
    knowledge_graph_inst,
    test_item: str,
) -> list[dict[str, Any]]:
    test_id = _stable_test_id(test_item)
    ...
    for src, tgt in edges:
        edge = await knowledge_graph_inst.get_edge(src, tgt)
        if edge and edge.get("rel_type") == "BASED_ON" and edge.get("src_id") == test_id:
            clause_ids.append(edge.get("tgt_id"))

    nodes = await knowledge_graph_inst.get_nodes_batch(clause_ids)
    return [node for node in nodes.values() if node is not None]
